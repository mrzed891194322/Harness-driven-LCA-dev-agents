...
3. 创建新的 `review_id`，格式为 `YYYYMMDDTHHMMSSZ-<8 位小写十六进制>`，目标目录固定为 `workspace/outputs/reports/lca-quality/<review_id>/`。目录已存在时生成新 ID，不覆盖历史。
...
2. 回读命中结果给出的 ISO 原文路径和定位；记录标准版本、原文路径和实际使用的条款。RAG 片段不是最终证据。
