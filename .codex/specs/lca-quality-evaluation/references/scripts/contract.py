...
def render_markdown(data: dict[str, Any], rubric: dict[str, Any] | None = None) -> str:
    ...
    lines = [
        ...
        _table(
            ["标准", "版本", "原文路径", "定位"],
            [[item["id"], item["edition"], item["path"], item["locators"]] for item in data["standards"]],
        ),
        "",
        "## 4. 输入证据与产物覆盖",
        "",
        _table(
            ...
            [[item["artifact_id"], item["kind"], item["path"], item["file_status"], item["schema_status"], item["details"]] for item in data["inputs"]],
        ),
        "",
        "### 固定产物覆盖",
        "",
