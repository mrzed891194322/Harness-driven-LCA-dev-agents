...
使用方式：
    # 同步当前分支
    uv run python scripts/gitee_upload/main.py
    
    # 同步所有分支和标签
    uv run python scripts/gitee_upload/main.py --all --tags
    
    # 强制同步指定分支
    uv run python scripts/gitee_upload/main.py --branch master --force
    
    # 演练模式
    uv run python scripts/gitee_upload/main.py --dry-run
"""
