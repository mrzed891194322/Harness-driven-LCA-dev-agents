...
# Script lives in scripts/debug_gui/, go up 2 levels to project root
...
$GuiScript = Join-Path $ProjectRoot 'GUI\main.py'
...
if ($busy) {
    ...
    Write-Host "Run this first: python scripts\debug_gui\stop_gui.py" -ForegroundColor Yellow
    Read-Host "Press Enter to exit"
    exit 1
}
