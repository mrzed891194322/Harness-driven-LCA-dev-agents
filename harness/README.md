...
LCA 主 YAML 注册的是 `src/scripts/workflows/domains/lca/openlca_mcp.py` 和 `artifacts/main.py` 两个适配入口。它们在底层工具之外实施审核批准、角色限制、证据归档与固定产物路径；独立工具仍在 `harness/tools/`，不依赖 workflow。`control_openlca.tool_timeout_sec` 显式设为 7320 秒；修改 IPC 会话预算时同步调整该值，留足约 120 秒缓冲。
