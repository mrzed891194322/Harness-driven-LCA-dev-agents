...
4. 每次委派必须明确要求子 Agent 读取本阶段 README/spec 与本次输入路径。子 Agent 不得再委派、不得扫描其他阶段。不要在委派 prompt 中抄规则路径。
...
- 委派任务必须明确要求 `sub-executor` 读取上述文件，写出 mapping 与 LCI。
...
- 委派任务必须明确要求 `sub-executor` 读取上述文件，按预检→导入→读回→计算→报告执行，模板为 `harness/specs/04-openlca-reporting/references/templates/lca_report.md`。
