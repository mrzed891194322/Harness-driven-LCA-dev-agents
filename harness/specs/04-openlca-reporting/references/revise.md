...
- 追加 `references/templates/revision-report-sections.md`：§7 本轮修订摘要、§8 用户意见落实矩阵、§9 与上一轮结果的差异。
- §8 必须覆盖 `revise.md` 中每条可执行意见；`REV-*` 指向 BOM `item_id` 或 mapping 行。
- §9 每个数值差异回链新旧 raw 结果路径；无法比较时说明原因，不得推断环境优势。
