...
已审 BOM JSON 与 Markdown（含需求/情景及数量依据）、有效计划/修订、声明资料和通过 control_openlca 查询的活动库。遵守共同方法、清单、映射与工具规则。

## 提交

`workspace/outputs/inventory/process-mapping.json` 保持 items 数组，每行对应 BOM item_id，包含选用 Flow / Process / Provider 的名称与 UUID、请求/实际地域、中文 selection_reason 和候选摘要。每个 BOM item_id 仍有且仅有一条映射；一条行涉及多个情景或 exchange 的对应关系写在 Markdown，不用复制 item_id 破坏覆盖关系。
...
`workspace/outputs/LCI/` 保持 flows/、processes/、product_systems/ 中一文件一实体的 JSON-LD，以及根目录 `human_readable_mapping.md`。
...
示例 `references/examples/process-mapping.json` 仅展示结构，不能将示例 UUID 当作查询结果。
