...
只有第 05 阶段当前预检哈希及完整范围得到明确确认后才能调用 `import_lci`。本阶段必须在 `workspace/results/<run_id>/` 保存：
...
- 无法识别 Product System、读回失败或 `broken_links` 非空时不得进入第 07 阶段；保存证据并置为 `failed` 或 `needs_review`。
- 只有导入零失败且模型图无断链时，才允许进入 LCIA 计算。
