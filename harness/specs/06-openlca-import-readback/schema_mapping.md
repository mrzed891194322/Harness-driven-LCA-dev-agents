...
| MCP schema | `import_lci(preflight_hash, lci_dir, target_category, database_name)` | 预检交接后 | `sub-executor` ↔ `control_openlca` MCP | 重算范围/hash 后导入，返回逐项实体状态、错误和计数 | 部分成功也置 `failed` |
...
| MCP schema | `get_model_graph(product_system)` | 导入零失败后 | `sub-executor` ↔ `control_openlca` MCP | 从活动数据库读回节点、边、断链和孤立节点 | Product System 不明或读回失败停止 |
| 产物 schema | `harness/specs/06-openlca-import-readback/references/schemas/model-graph.schema.json` | 保存模型图前 | MCP 原始返回 → model graph JSON | Product System ref、nodes、edges、broken/disconnected、状态和时间 | 空节点、断链或孤立节点置 `failed` |
...
- 两个产物 schema 只属于 06；字段、版本或路径变化时同步 MCP 转换逻辑、阶段 spec、workflow adapter、质量 rubric 和契约测试。
