...
- **按需调用双库检索能力**：
   ...
   - **外部背景数据**：当需要将原材料或能源挂载到实际的背景数据集（如 ecoinvent）时，**必须加载并调用** `external-tools` 技能路由到 `harness/tools/control_openlca/query_descriptors/`，检索获取目标过程的 UUID 并填入 `defaultProvider`，严禁凭空捏造。
