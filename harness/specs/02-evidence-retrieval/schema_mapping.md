...
| MCP schema | `health_check()`、`query_descriptors(...)`、`get_process_details(...)`、`get_flow_providers(...)` | 查询 Process/Flow/Product System/Impact Method 候选、Process 定量参考及 Flow 的可用 Provider | `sub-executor` ↔ `control_openlca` MCP | 按公共运行契约执行 openLCA 连接门禁，再记录 entity type、查询词、分页、名称、UUID、分类、地域、定量参考、Flow-Provider 引用和查询时间 | 连接失败按公共运行契约停止；不得降级为未解决项 |
