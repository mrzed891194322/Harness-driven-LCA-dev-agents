...
- 比较情景必须核对 auto-link 拓扑中的前景输入 `defaultProvider`、Provider 输出 Flow、`expectedProcessIds`、背景 Provider flow 与地域证据；只检查 JSON 字段存在不足以通过。第 02 阶段已记录、且 UUID 与输出 Flow 一致的匹配决定不是阻断问题；审查只需确认细节已写入映射报告。
...
- attempt 3 未通过：不得进行第 4 次修正或审查；保存问题，manifest 置为 `failed`，在 `status_reason` 写明未解决 issue ID 并停止。
