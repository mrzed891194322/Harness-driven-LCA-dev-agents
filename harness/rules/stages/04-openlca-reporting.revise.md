...
- 上一轮 `lca_report.md` 及其 raw 结果（若仍存在）
...
- §9 必须覆盖 `revise_plan.md` 中每条可执行意见；`REV-*` 指向 BOM `item_id`、mapping 行或分析证据。共有需求落实表仍覆盖未被修订的原计划要求。
- §10 每个数值差异回链新旧 raw 结果路径；比较前核对功能单位、范围与方法是否一致，无法比较时说明原因，不得推断环境优势。

## 审查补充

矩阵与 `revise_plan.md` 逐条对应优先于文风或章节完整度：意见未落实不得 `passed`。因落实意见而相对原报告改写模型或数值不得失败，但数值仍须能指到本轮 MCP 原始返回。工具报错、空结果或资源未释放仍不得通过。`fix_instructions` 先写未落实的意见与 `REV-*`，再写缺失章节或原始文件。
