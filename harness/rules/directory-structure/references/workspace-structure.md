...
├── memory/                 # 当前运行证据；revise-lca 可含直接上一轮 baseline/
├── outputs/
...
* **运行记忆 (`memory/`)**：固定保存 `manifest.json` 以及 `stages/`、`reviews/`、`handoffs/`。revise-lca 在 `baseline/` 保存直接上一轮的只读 plan、LCI、reports 和 memory，不递归保留更早 baseline。不同 Agent 可按阶段任务读取相关记忆；主编排 Agent 负责写入和维护关联。
