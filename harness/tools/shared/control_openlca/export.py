...
def print_results_table(formatted_results):
    ...
    print("\n### 计算结果总览 (LCIA Results)")
    print(
        "| 影响类别 (Impact Category) | 数值 (Amount) | 单位 (Unit) | 影响分类 UUID (UUID) |"
    )
    print("| --- | --- | --- | --- |")
    for r in formatted_results:
        print(
            f"| {r['category_name']} | {r['amount']:.6e} | {r['unit']} | {r['category_id']} |"
        )
    print()
    ...
    if ext == ".json":
        ...
        try:
            with open(output_path, "w", encoding="utf-8-sig", newline="") as f:
                ...
                for r in formatted_results:
                    writer.writerow(
                        [r["category_name"], r["amount"], r["unit"], r["category_id"]]
                    )
        ...
        print(
            f"[警告] 不支持的导出格式 '{ext}'，仅支持 .json 或 .csv 后缀。跳过文件导出。"
        )
