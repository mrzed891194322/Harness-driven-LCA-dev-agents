def add_arguments(parser):
    """
    配置并添加用于过程直接计算的专用命令行参数。
    """
    parser.add_argument("process", type=str, help="目标过程 (Process) 的名称或 UUID")
    parser.add_argument(
        "--method",
        "-m",
        type=str,
    ...
    parser.add_argument(
        "--regionalized", "-r", action="store_true", help="启用区域化计算"
    )
    parser.add_argument("--costs", "-c", action="store_true", help="启用成本核算")
    parser.add_argument(
        "--parameter",
        "-param",
        action="append",
