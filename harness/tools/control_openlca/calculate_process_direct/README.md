...
```bash
uv run python harness/tools/control_openlca/calculate_process_direct/main.py "过程名称或UUID" --method "LCIA方法名称或UUID" [可选参数]
```
...
```bash
uv run python harness/tools/control_openlca/calculate_process_direct/main.py "Process B" --method "EF 3.1"
```

### 2. 带有分配方法与结果导出的高级计算
使用经济分配计算过程，并将结果导出为 CSV 文件：
```bash
uv run python harness/tools/control_openlca/calculate_process_direct/main.py "Process B" -m "EF 3.1" -al economic -o output/process_results.csv
```
