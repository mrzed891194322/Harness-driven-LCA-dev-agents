...
- **环境与代码规范**：若涉及编写脚本或进行任何文件操作（如复制、移动、删除等），必须严格参考并遵守 `project-regulation` 技能的 `assets/coding-specification/` 中的代码编写与运行规范（包括作用域限制、Python 脚本必须运行在本地 uv 虚拟环境中，临时 Python 脚本须在 `src/tmp/` 中等）。
