...
- **模板约束**：当生成或更新有模板要求的文档时，**必须显式读取**相关技能或其它 Agent 传递的信息/指令中所指定的模板文件，并以此为结构基准来写入或更新文档（例如处理 LCA 计划时，显式读取并参考 `project-specification` 技能的 `assets/plan-specification/` 目录的模板）。
