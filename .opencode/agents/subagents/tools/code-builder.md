...
- 保持改动范围最小，优先复用仓库已有模式。
- 不覆盖用户已有改动；修改前读取目标文件。
- 代码编写、目录组织、模块职责、文件命名和交付物位置必须遵守 `project-regulation` 技能的 `assets/project-convention/` 目录的项目目录规范；开始实现前若不确定结构，先读取该 skill 及其按需引用的规范文档。
- **必须严格阅读并遵守 `programming-standards`（编程规范）技能。** 包括代码拆分、调用 `doc-handler` 更新目录文档、以及优先复用历史脚本。
- 不允许创建新的代码目录来绕开既有规范，例如 `app/`、`core/` 等未在 `project-regulation` 技能的 `assets/project-convention/` 目录的项目目录规范中规定的目录。
