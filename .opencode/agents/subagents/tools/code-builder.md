...
- **必须严格遵守 `project-regulation` 技能定义的所有规范：**
  ...
  - **代码编写与运行规范**：必须严格查阅并遵守 `project-regulation` 技能的 `assets/coding-specification/instructions/python_specification.md` 和 `general_specification.md`，包括运行环境 (.venv)、作用域边界限制 (`GUI/`、`harness/tools/`、`scripts/` 与 `workspace/` 目录内)、临时脚本规范 (`workspace/tmp/`) 以及优先复用历史脚本的原则。
