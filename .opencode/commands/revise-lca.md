...
- GUI 或用户已在启动前通过 `src/scripts/clean_dir/main.py -y --preset revise-lca` 清理 knowledge 与 openLCA（不清理 workspace），并把意见写到 `workspace/inputs/revise.md`；**不要**在 agent 内调用 `clean_dir` 或 MCP `cleanup_output`。
