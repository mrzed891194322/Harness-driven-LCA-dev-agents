...
description: 使用此技能将指定输入目录（默认 input/）中的 PDF、Word、Markdown 和文本文件等通过 markitdown 处理并提取文本，然后将其嵌入（embeddings）存储在指定输出目录（默认 src/knowledge/）的 Chroma RAG 数据库中。
---
...
如果未提供参数，将默认使用 `input` 作为输入目录，`src/knowledge` 作为输出目录。
