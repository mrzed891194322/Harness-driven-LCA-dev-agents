# openLCA 控制资产说明及公共工具规范 (README.md)

本目录为 `control-openlca` 技能的资产目录。为了保证代码复用性、降低维护成本，本技能采用了“**共享公共工具包 + 专属私有任务包**”的架构设计。
...
1.  **首选复用**：当开发新脚本时，主程序顶部必须通过追加 `sys.path` 导入 `assets/utils/` 下的对应功能。
2.  **单一职责**：请勿在新脚本主文件中编写关于连接、查找、导出等繁琐实现。`main.py` 应当只负责顶层流程编排。
3.  **升级与扩展**：
    *   如果需要对通用逻辑（如引入新的结果展现形式）进行调整，**应当直接修改 `assets/utils/` 下的对应模块**，确保全技能通用逻辑同步升级。
