...
* 待完善清单 (`todo_list.md`) 目前把"缺失数据"和"决策事项"混在一起。**是不是把这两类分开更好**——比如把必须做决策的事项（分配方式、截断规则等）和纯数据缺口区分开？（清单的结构在技能规范的 `template/` 中定义，可以调整分类方式。）
* 映射报告 (`human_readable_mapping.md`) 列出了各风险项，但**没有直接说应该改哪个 JSON 文件的哪个字段**。需不需要这种精确到文件和字段级别的修改指引？还是更倾向直接在 openLCA 软件里操作，不需要碰 JSON？
