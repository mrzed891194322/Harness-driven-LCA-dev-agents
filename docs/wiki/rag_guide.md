...
Agent 会根据您的指令识别意图及路径参数，自动调用相应的 `external-tools` 技能（具体参考 `assets/control-rag-database/README.md`）在指定的目标目录下构建向量数据库。

### 方式三：直接运行 Python 脚本
若需要在项目虚拟环境中进行更灵活的参数配置，可直接运行 Python 脚本：
```bash
uv run python .opencode/skills/external-tools/assets/control-rag-database/scripts/build_rag/main.py --input-dir <输入目录路径> --output-dir <输出目录路径>
```
**常用参数**：
...
Agent 会识别您的意图，自动调用 `external-tools` 技能（具体参考 `assets/control-rag-database/README.md`）进行知识检索，并结合检索结果回答您的问题或执行后续任务。

### 方式二：直接运行 Python 脚本
您也可以直接在终端运行查询脚本，传入查询字符串以手动检索数据库内容：
```bash
uv run python .opencode/skills/external-tools/assets/control-rag-database/scripts/query_rag/main.py "您的查询关键词或问题" --db-dir <RAG数据库目录>
```
**常用参数**：
- `--db-dir` 或 `-d`：要查询的 RAG 数据库目录（默认值为 `src/knowledge`）。
- *示例*：
  ```bash
  uv run python .opencode/skills/external-tools/assets/control-rag-database/scripts/query_rag/main.py "生命周期评价标准" --db-dir src/knowledge/standards
  ```
