...
from gui.functions.utils.path_utils import find_project_root
...
def execute_command_stream(
    ...
    *,
    ...
    from gui.functions.utils.process_manager import (
    ...
    try:
        ...
        if not completed and process.poll() is None:
            try:
                ...
                safe_console_print(
                    f"[Process Manager] Error cleaning up command process: {e}"
                )
        clear_active_process()
...
def clean_dir_command(
    *,
    ...
    clean_staging: bool | None = None,
) -> list[str]:
    if clean_staging is None:
        clean_staging = CLEAN_GUI_STAGING
    ...
    if not clean_staging:
        command.append("--no-staging")
    return command


CLEAN_WORKSPACE_COMMAND = clean_dir_command(target="workspace", clean_staging=True)
...
def _stream_subprocess_command(
    ...
    *,
    ...
    from gui.functions.utils.process_manager import should_stop
...
def run_clean_preset_console(
    ...
    *,
    ...
    if clean_staging is None:
        clean_staging = CLEAN_GUI_STAGING
    suffix = "" if clean_staging else " --no-staging"
    yield from _stream_subprocess_command(
        clean_dir_command(preset=preset, clean_staging=clean_staging),
        start_message=f"[System] 正在执行 clean_dir --preset {preset}{suffix}...\n",
    )
...
def run_pre_workflow_console(
    ...
    *,
    ...
    from gui.functions.file_sync.main import sync_files
...
def run_workflow_command_console(
    ...
    """
    from gui.functions.settings.settings import load_harness_agent
