...
import harness.tools.mcp.control_openlca.workflow_mcp as mcp_module
from harness.tools.mcp.control_openlca.utils import connection, readonly
from tests.support.openlca_fakes import FakeClient


def _tool_annotations(tool: object):
...
class ReadOnlyServiceTests(unittest.TestCase):
    def test_build_endpoint_validates_and_formats_hosts(self) -> None:
        self.assertEqual(
            connection.build_endpoint("localhost", 8080), "http://localhost:8080"
        )
...
class MCPServerTests(unittest.TestCase):
    def setUp(self):
        ...
        env = patch.dict(
            os.environ, {"LCA_WORKSPACE": temporary.name, "LCA_RUN_ID": "test-run"}
        )
        env.start()
    ...
    def test_workflow_tools_are_registered_with_safe_annotations(self) -> None:
        ...
        self.assertTrue(_tool_annotations(tools["get_flow_providers"]).read_only_hint)
        self.assertFalse(
            _tool_annotations(tools["get_process_details"]).destructive_hint
        )
        self.assertFalse(
            _tool_annotations(tools["get_flow_providers"]).destructive_hint
        )
        self.assertTrue(_tool_annotations(tools["preflight_import_lci"]).read_only_hint)
        self.assertFalse(
            _tool_annotations(tools["preflight_import_lci"]).destructive_hint
        )
        ...
        self.assertEqual(
            tools["preflight_import_lci"].input_schema["properties"]["lci_dir"][
                "default"
            ],
    ...
    def test_workflow_lci_dir_accepts_canonical_and_tmp_subdirectory(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with (
                patch.object(mcp_module, "PROJECT_ROOT", project_root),
                patch.dict(
                    os.environ, {"LCA_WORKSPACE": str(project_root / "workspace")}
                ),
            ):
                self.assertEqual(
                ...
                with self.assertRaisesRegex(
                    ValueError, "subdirectory of workspace/tmp"
                ):
                    mcp_module._workflow_lci_dir("workspace/outputs/LCI")
                with self.assertRaisesRegex(
                    ValueError, "subdirectory of workspace/tmp"
    ...
    def test_health_tool_uses_configured_endpoint(self) -> None:
        ...
        with (
            ...
            patch.object(
                mcp_module, "run_health_check", return_value=expected
            ) as health,
        ):
    ...
    def test_invalid_configured_port_is_rejected(self) -> None:
        ...
        self.assertEqual(result["errors"][0]["kind"], "invalid_input")
