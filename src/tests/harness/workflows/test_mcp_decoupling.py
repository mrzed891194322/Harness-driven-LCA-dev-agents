...
from scripts.agent_sdk.providers.claude.session import write_claude_mcp
from scripts.agent_sdk.providers.codex.session import mcp_overrides
from scripts.agent_sdk.providers.opencode.session import write_opencode_mcp
from scripts.agent_sdk.providers.pi.session import write_pi_mcp
from scripts.workflows.domains.lca.bootstrap import validate_handoff
...
def test_check_snapshot_does_not_hide_changes_or_leak_between_calls(tmp_path):
    from scripts.workflows.domains.lca.artifacts.snapshot_io import (
        check_snapshot,
        load_json,
        sha256_file,
    )
...
def test_workflow_adapter_entrypoints_preserve_context_and_response(tmp_path, tool):
    ...
    base = PROJECT_ROOT / "src/scripts/workflows/domains/lca"
    if tool == "artifacts":
