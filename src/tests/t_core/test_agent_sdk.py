...
from core.agents.providers.claude.session import (
    ClaudeSessionProvider,  # noqa: E402
    write_claude_mcp,
)
from core.agents.providers.cli_base import CliRunResult  # noqa: E402
from core.agents.providers.codex.session import (  # noqa: E402
    CodexSessionProvider,
    mcp_overrides,
)
from core.agents.providers.opencode.session import (
    OpenCodeSessionProvider,  # noqa: E402
)
from core.agents.session import (  # noqa: E402
    SessionConfig,
    SessionError,
    SessionRef,
    SessionResumeError,
...
class AgentSdkSessionTests(unittest.TestCase):
    ...
    def test_pi_run_turn_passes_model_and_mcp(self) -> None:
        ...
        from core.agents.providers.pi.session import PiSessionProvider
...
class AgentSdkCatalogTests(unittest.TestCase):
    ...
    def test_list_pi_models_keeps_provider_prefix(self) -> None:
        from core.agents.catalog import list_models, split_pi_model_ref
    ...
    def test_list_pi_models_keeps_provider_prefix_on_collision(self) -> None:
        from core.agents.catalog import parse_pi_models
...
class AgentSdkProgressTests(unittest.TestCase):
    def test_session_tag_and_header_own_line(self) -> None:
        from core.agents.progress import format_tagged, session_tag
    ...
    def test_orchestrator_tag_stays_inline_when_short(self) -> None:
        from core.agents.progress import format_tagged, orchestrator_tag
    ...
    def test_print_orchestrator_mirrors_to_progress_log(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                with patch("core.agents.progress.clock", return_value="22:01:25"):
                    print_orchestrator("start run_id=abc")
                text = log_path.read_text(encoding="utf-8")
                set_progress_log(log_path, append=True)
                with patch("core.agents.progress.clock", return_value="22:01:26"):
    ...
    def test_opencode_pi_claude_key_formatters(self) -> None:
        from core.agents.providers.claude.jsonl import ClaudeJsonlFormatter
        from core.agents.providers.opencode.jsonl import OpenCodeJsonlFormatter
        from core.agents.providers.pi.jsonl import PiJsonlFormatter
    ...
    def test_tool_lines_include_args_not_results(self) -> None:
        from core.agents.progress import format_tagged, format_tool_start
        from core.agents.providers.claude.jsonl import ClaudeJsonlFormatter
        from core.agents.providers.opencode.jsonl import OpenCodeJsonlFormatter
        from core.agents.providers.pi.jsonl import PiJsonlFormatter
    ...
    def test_parse_claude_stream_json_and_single_object(self) -> None:
        from core.agents.providers.claude.session import parse_claude_output
    ...
    def test_run_turn_prints_session_header_on_own_line(self) -> None:
        ...
        from core.agents.providers.codex.session import CodexSessionProvider
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            ref = provider.create(config)
            with (
                patch("core.agents.progress.clock", return_value="19:33:12"),
                patch("core.agents.providers.cli_base.print_session") as printed,
            ):
                provider.run_turn(ref, "hello", config)
