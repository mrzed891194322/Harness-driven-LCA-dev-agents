...
GUI_CONTROL_DIR = PROJECT_ROOT / "src" / "scripts" / "gui_control"
if str(GUI_CONTROL_DIR) not in sys.path:
    sys.path.insert(0, str(GUI_CONTROL_DIR))

import gc_utils.process as process_mod  # type: ignore[import-not-found]
from gc_utils.process import (  # type: ignore[import-not-found]  # noqa: E402
...
from stop_gui import stop_gui  # type: ignore[import-not-found]
...
class ProcessRecordTests(unittest.TestCase):
    ...
    def test_write_record_keeps_starttime(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            path = Path(temp_dir) / "gui.pid"
            with (
                patch.object(process_mod, "snapshot_process", return_value=snapshot),
                patch.object(process_mod, "port_listeners", return_value=[]),
            ):
...
class LiveMatchTests(unittest.TestCase):
    def test_pid_reuse_without_matching_starttime_is_rejected(self) -> None:
        ...
        with (
            patch.object(process_mod, "is_process_alive", return_value=True),
            patch.object(process_mod, "_comm", return_value="python"),
...
class KillSafetyTests(unittest.TestCase):
    def test_pid_one_never_signaled(self) -> None:
        with (
            patch.object(process_mod.os, "kill") as mock_kill,
            patch.object(process_mod.os, "killpg", create=True) as mock_killpg,
        ):
            kill_process_tree(1)
            kill_process_tree(0)
    ...
    def test_recorded_kill_skipped_on_identity_mismatch(self) -> None:
        ...
        with (
            patch.object(process_mod, "live_matches", return_value=False),
            patch.object(process_mod, "_kill_pid_tree") as mock_kill,
        ):
    ...
    def test_unix_tree_kill_never_uses_killpg(self) -> None:
        with (
            patch.object(process_mod, "is_gui_process", return_value=True),
            patch.object(process_mod, "_ancestor_pids", return_value=set()),
            patch.object(process_mod, "_collect_tree", return_value=[222, 111]),
            patch.object(process_mod, "is_process_alive", return_value=False),
            patch.object(process_mod.sys, "platform", "linux"),
            patch.object(process_mod.os, "kill") as mock_kill,
            patch.object(process_mod.os, "killpg", create=True) as mock_killpg,
        ):
...
class StopGuiRecordTests(unittest.TestCase):
    def test_stale_identity_does_not_kill(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with (
                patch("stop_gui.PID_FILE", path),
                patch(
                    "stop_gui.kill_recorded_target", return_value=False
                ) as mock_kill_recorded,
                patch("stop_gui.port_listeners", return_value=[]),
