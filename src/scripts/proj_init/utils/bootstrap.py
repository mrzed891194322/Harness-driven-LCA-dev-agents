...
def check_uv(
    which: WhichFn = shutil.which, run: RunFn = subprocess.run
) -> dict[str, Any]:
...
def _inspect_worker(name: str) -> tuple[bool, str]:
    from scripts.agent_sdk.inspect import inspect as inspect_worker
...
def run_bootstrap(
    *,
    ...
    import fails. Missing harness SDKs do not fail the run.
    """
    ...
    report["harness_clis"] = detect_harness_clis(inspect_fn=inspect_fn)
