...
class SetupEnvBootstrapTests(unittest.TestCase):
    ...
    def test_detect_harness_clis_lists_supported_workers(self) -> None:
        ...
        with patch.object(bootstrap_mod, "_antigravity_available", return_value=False):
            report = detect_harness_clis(which=fake_which)
        self.assertEqual(
            set(HARNESS_CLIS),
            {"opencode", "claude", "codex", "dsh", "antigravity"},
        )
        ...
        self.assertFalse(report["clis"]["antigravity"]["available"])
