...
.DESCRIPTION
    ...
    This script lives under src/scripts/debug_gui/, alongside start_gui.py etc.
    ProjectRoot is 3 levels up (debug_gui -> scripts -> src -> project root).
...
# Script lives in src/scripts/debug_gui/, go up 3 levels to project root
...
if ($busy) {
    ...
    Write-Host "Run this first: python src\scripts\debug_gui\stop_gui.py" -ForegroundColor Yellow
    Read-Host "Press Enter to exit"
    exit 1
}
