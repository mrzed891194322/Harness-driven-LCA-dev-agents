...
def main():
    ...
    parser.add_argument("--all", action="store_true", help="同步本地所有分支到 Gitee")
    parser.add_argument("--tags", action="store_true", help="同步本地所有标签到 Gitee")
    parser.add_argument(
        "--branch",
        type=str,
    ...
    # 1. 检查和配置远程源
    if not check_and_setup_gitee_remote(
        PROJECT_ROOT, GIT_REMOTE_NAME, DEFAULT_GITEE_URL, dry_run=args.dry_run
    ):
    ...
    if args.force and not args.yes and not args.dry_run:
        print("\n" + "!" * 60)
        print(
            "警告：您启用了强制推送 (--force)！这可能会覆盖远程仓库的提交记录，导致数据丢失！"
        )
        print("!" * 60)
