...
def check_and_setup_gitee_remote(
    project_root: Path, remote_name: str, default_url: str, dry_run: bool = False
) -> bool:
    """
    ...
    if remote_name not in remotes:
        ...
        add_success, add_output = run_git_command(
            ["remote", "add", remote_name, default_url], project_root
        )
        if not add_success:
    ...
    # 检查已有的 URL
    url_success, url_output = run_git_command(
        ["remote", "get-url", remote_name], project_root
    )
    if not url_success:
    ...
    if current_url != default_url:
        ...
        if dry_run:
            ...
            return True
        set_success, set_output = run_git_command(
            ["remote", "set-url", remote_name, default_url], project_root
        )
        if not set_success:
