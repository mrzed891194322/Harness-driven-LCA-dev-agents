...
批量调用 src/tools/control_rag_db 中的 build_rag 工具构建 RAG 知识库。
"""
...
# ── 注入工具路径 ──────────────────────────────────────────────────────────────
# src/scripts/initialization/rag_init/main.py
#   parents[0] = rag_init/
#   parents[3] = src/
_BUILD_RAG_DIR = Path(__file__).parents[3] / "tools" / "control_rag_db" / "build_rag"
_CTRL_RAG_DIR  = Path(__file__).parents[3] / "tools" / "control_rag_db"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="按映射规则批量构建 RAG 知识库（调用 src/tools/control_rag_db/build_rag）"
    )
    parser.add_argument(
        "--project-root",
        type=str,
        default=None,
        help="项目根目录（默认自动推断为本脚本的 parents[4]）"
    )
    parser.add_argument(
        "--clean",
        action="store_true",
    ...
    if args.project_root:
        ...
        # src/scripts/initialization/rag_init/ -> parents[4] = 项目根
