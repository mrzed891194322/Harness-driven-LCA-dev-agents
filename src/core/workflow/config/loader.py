...
REGISTRY_KEYS = frozenset(
    {"rules", "tools", "knowledge", "checkers", "hooks", "handoff_validators"}
)
DEFAULTS_KEYS = frozenset({"rules", "knowledge"})
HOOKS_KEYS = frozenset({"on_reviewer_passed"})
PROVIDER_ENTRY_KEYS = frozenset({"provider"})
TOOL_KEYS = frozenset(
    {
        "transport",
        "command",
...
def _merge_workflow(
    ...
    if "registry" in extra:
        ...
        for section in ("checkers", "hooks", "handoff_validators"):
            if section in overlay_registry:
                result["registry"].setdefault(section, {}).update(
                    overlay_registry[section] or {}
                )
    if "hooks" in extra:
...
def _parse_workflow(raw: dict[str, Any], *, source_path: Path) -> Workflow:
    ...
    if "capabilities" in raw:
        raise ValueError(
            f"{source_path}: 'capabilities' was removed; declare "
            f"registry.checkers / registry.hooks / registry.handoff_validators "
            f"with provider: module:callable instead"
        )
    ...
    checker_providers = _parse_provider_section(
        registry.get("checkers"),
        section="checkers",
        source_path=source_path,
    )
    hook_providers = _parse_provider_section(
        registry.get("hooks"),
        section="hooks",
        source_path=source_path,
    )
    handoff_validator_providers = _parse_provider_section(
        registry.get("handoff_validators"),
        section="handoff_validators",
        source_path=source_path,
    )
    ...
    return Workflow(
        ...
        hook_providers=hook_providers,
        handoff_validator_providers=handoff_validator_providers,
    )
...
def _parse_provider_section(
    ...
    *,
    ...
    if not isinstance(raw, dict):
        raise ValueError(f"{source_path}: registry.{section} must be a mapping")
    out: dict[str, str] = {}
    for key, spec in raw.items():
        eid = require_identifier(str(key), label=f"{section} id")
        if not isinstance(spec, dict):
            raise ValueError(
                f"{source_path}: registry.{section}.{eid} must be a mapping"
            )
        reject_unknown_keys(
            spec,
            PROVIDER_ENTRY_KEYS,
            f"{source_path}: registry.{section}.{eid}",
        )
        provider = spec.get("provider")
        if not isinstance(provider, str) or not provider.strip():
            raise ValueError(
                f"{source_path}: registry.{section}.{eid}: "
                f"provider must be a non-empty 'module:callable' string"
            )
        out[eid] = provider.strip()
    return out
